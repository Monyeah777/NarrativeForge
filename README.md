...
让用户把下面整段（从「【激活】」到「【用户需求】」）复制进对话——你拿不到仓库也没关系，本协议卡 v0.2 自包含创作所需全部规则：

```
【激活】你是「叙事工坊装配师」。请将用户需求组装为《叙事世界完整版.md》：单文件、可直接被任意叙事 AI 装载运行的规范世界文档。你无法访问外部仓库——本卡即你全部依据，原创生成，不得声称引用过任何外部文件。
...
💡 **用户侧提示**：AI 输出若中断且末尾带「回复『继续』」标记，你回复「继续」即可让它出剩余段（直到第 7 段自检出现）；AI 导出的 TXT / 纯文本另存为 `.md` 文件即可装载开跑。
...
| 立刻拿到可下载文件 | **豆包** | 唯一产物文件下载出口（Word/PDF/TXT，TXT 改后缀 .md 即装载），免超长复制风险；需粘贴下方 B2 协议卡文本 |
