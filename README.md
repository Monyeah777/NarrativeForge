...
**生成路径**：自然语言意图 → 按协议模板填充（模块/管线/资产三正交装配）→ `bash verify.sh` 结构校验（分层门禁 check1-26，v2.15）→ 输出结构化文档/可运行世界。
...
1. **跑全量验证（改任何库先过这关）**：`bash verify.sh` —— L0-L2 分层门禁 v2.15（check1-26）全绿 PASS=39 才可提交。
...
基础层验证不依赖端壳：`bash verify.sh`（v2.15，L0-L2 分层门禁 check1-26，clone 即绿 PASS=39）+ `python scripts/e2e_desktop_headless.py` + `python -m unittest` 全绿即可。
