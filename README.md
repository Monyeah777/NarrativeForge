...
> **v0.7.0 自定义协议**（当前主线·开发中）：在 v0.6 统一入口之上对外开放协议注册——第三方按 protocol.yaml 协议声明注册自定义协议（01 §6 Schema 规范 → 02 §8.3 登记三要件 + registry.json protocols[] 投影 → verify.sh check14 注册门禁，v2.3 → v2.4）；版本演进与任务记录见 ROADMAP 与 10 方案。
