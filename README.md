...
## 协议链
01_核心协议 → 02_联动注册表 → 03_管线库 → 04_模块库 → 05_资产库 → 06_Agent执行协议 → 07_官方核心出厂与社区预设导航 → 08_v0.5.0_优化版方案 → 09_v0.6.0_协议中转站方案。
> **v0.6.0 协议中转站**（当前主线）：协议统一入口（registry.json 机读投影 + registry_loader 装载器）→ 迁移机制落地（01 §7 V2 bump 首例）→ sync --check 差异校验 → 工程治理补漏。verify.sh v2.3 三段式门禁（check1–13）；版本演进与任务记录见 ROADMAP 与 09 方案。
