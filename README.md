...
**生成路径**：自然语言意图 → 按协议模板填充（模块/管线/资产三正交装配）→ `bash verify.sh` 结构校验（十二项门禁，check1-12）→ 输出结构化文档/可运行世界。
...
> **v0.5.0 优化版**（当前主线）：T1 编号治理 → T2 社区包装配 → T3 记忆分层 → T4 M80 门禁 → T5 资产工作台化 → T6 发布规范收口。版本演进、任务记录与门禁清单见 08 方案。
