...
**生成路径**：自然语言意图 → 按协议模板填充（模块/管线/资产三正交装配）→ `bash verify.sh` 结构校验（分层门禁 check1-32，v2.22）→ 输出结构化文档/可运行世界。
...
1. **跑全量验证（改任何库先过这关）**：`bash verify.sh` —— L0-L2 分层门禁 v2.22（check1-32）全绿 PASS=51 才可提交。
...
基础层验证不依赖端壳：`bash verify.sh`（v2.22，L0-L2 分层门禁 check1-32，clone 即绿 PASS=51）+ `python scripts/e2e_desktop_headless.py` + `python -m unittest` 全绿即可。
...
- 分层门禁：`bash verify.sh`（v2.22，check1-32 全绿 PASS=51，clone 即绿；Windows 用 Git Bash / WSL 跑 `bash`）
