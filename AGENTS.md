...
- **沙箱意识**:工具执行在项目目录内。路径逃逸（`../../etc/passwd`）的拦截由**宿主 harness 的工具层**负责——本仓库不对该层作实现承诺（此前写作「被 `validatePath` 拦截」而仓库内并无该实现,属**虚假保证**,2026-09-24 改动:文档只写真实载体）。仓库内组件之间的包含性判据 = `desktop/src/core/paths.py::validate_path`（资产台账已收敛到它,有单测）。若产生绕过的逃逸路径,拒绝执行。
...
### NF 方向与发布纪律（2026-09-08 作者批准）

- **开工前必读链**：`AGENTS.md`（本文件）→ `STRATEGY.md`（目标与质量准绳，最高层）→ `40_未来计划总纲_*.md`（路线）→ 目标文件。任何规划与 STRATEGY 冲突时，以 STRATEGY 为准。
- **质量法官 = 内部**：合格判据只来自 verify/audit/五维自评/真实战例；外部项目与外部 AI 分析仅作机制借鉴（逐条过仓库实证），**不得作为立项理由或质量背书的论据**。
- **计划内部消化**：不自行立项、不新建规划类文档推公开仓库；计划期产物（规划/草案/标准集/工作日志）留本地内部档案，落地后以结果形态（代码 + 收口注记 + audit）入库。
- **公开文案纪律**：仓库文档只承载结果/状态/正式协议；不写决策过程、不引作者原话、不写内部流水账。
- **封闭式发育**：外部实测线（E1/E2/Y5/B1 触发实测等）封存挂账；**一次性完整外部验证仅由作者显式触发**，agent 不主动安排/推进外部实测。
- 方向与验收边界见 `STRATEGY.md`；与之冲突时以 STRATEGY + 本条为准。
