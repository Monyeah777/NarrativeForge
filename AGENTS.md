...
### 公开仓边界与封闭式发育（2026-09-08 战略裁决）

- 计划类文档（规划/草案/标准集/工作日志）一律留本地工作区 `.rivet/`（不入库、不同步云端）；公开仓库只承载 `STRATEGY.md`（方向）+ 结果/状态（CHANGELOG/README 版本块/audit/收口文档）+ 正式协议代码。**agent 不得自行立项发公开规划**。
- 封闭式发育期：外部实测线（E1/E2/Y5/B1 触发实测等）封存挂账；**一次性完整外部验证仅由作者显式触发**，agent 不主动安排/推进外部实测。
- 公开文案纪律：只写「结果/状态」——禁止作者拍板原话、AI 协作工作流细节（agent 分工/实证过程）、内部流水账入公开仓库。
