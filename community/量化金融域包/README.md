...
| 资产 | **2 文件**：`assets/QUANT_GRAPH.md`（概念前置图：3 支 **27 概念** + 1 包外前置族 + 别名表）· `assets/QUANT_METRICS.md`（绩效与风控口径表：指标定义 / 口径纪律 / 常见口径事故） |
...
## 4. 装载命令（Agent 通道）
```
读 community/量化金融域包/README.md → 按 §2 表装载 P08 管线（P40 量化金融:M31 / P60 量化金融:M32）→ 读 assets/QUANT_GRAPH.md 机器可读块（3 支 27 概念 + 别名表）与 assets/QUANT_METRICS.md 口径表 → 按 §3 命令取前置闭包 / 缺失清单 / 就绪清单
```
## 5. 边界与不宣称
