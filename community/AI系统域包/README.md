...
| 资产 | **1 文件**：`assets/CONCEPT_GRAPH.md`（概念前置偏序图：24 概念 + 1 包外前置族，人读表 + 机器可读块双形态同源） |
| 主轴 | AI系统域：概念图 `CONCEPT_GRAPH`（P40 读图求闭包）→ M26 拓扑装载序与就绪门（P60）→ M80 官方输出门（P80，gate 唯一出口） |
| 输出风格 | M80 官方输出门（gate 唯一出口）；本包产物（闭包 / 缺失清单 / 装载序）供装配方与课程排程类消费者读取 |
...
| P60 长期演变 | **AI系统:M26** | 装载序与就绪门：`toposort` 确定性装载序 + 逐概念就绪门（load_order_ready） | 本包 |
...
> 求值器只读、确定性、零网络（同输入同输出）；序校验 `--order cmu-mlsys` / `--order aisystem-module-order` 给出「某份序列对图有多少违反边」。资产与实测数字见 `assets/CONCEPT_GRAPH.md` §5、记录见 `results/audit/docs_audit-50-ai-domain.md`。
## 4. 装载命令（Agent 通道）
```
读 community/AI系统域包/README.md → 按 §2 表装载 P07 管线（P40 AI系统:M25 / P60 AI系统:M26）→ 读 assets/CONCEPT_GRAPH.md 机器可读块 → 按 §3 命令取三件产物
```
## 5. 边界与不宣称
