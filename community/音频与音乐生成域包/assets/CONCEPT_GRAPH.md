...
> 覆盖：12 个包内概念 + 7 个**可扩展标准节点**（标准目录绑定）+ 1 个包外前置族（A05-00 领域通用前置）。
...
| `STD-cncf-cloudevents` | 标准 · CloudEvents 1.0（CNCF） | P80 | `A05-06` | std-catalog |
| `STD-creativecommons` | 标准 · 许可与权利表达（Creative Commons） | P80 | `A05-09` | std-catalog |
| `STD-ietf-json-schema` | 标准 · JSON Schema 2020-12（IETF/JSON Schema） | P80 | `A05-04` | std-catalog |
| `STD-mlcommons-bench` | 标准 · MLPerf 基准（可扩展场景）（MLCommons） | P80 | `A05-02`、`A05-08`、`A05-12` | std-catalog |
| `STD-onnx` | 标准 · ONNX（opset 扩展）（Linux Foundation） | P80 | `A05-03` | std-catalog |
| `STD-opengeospatial` | 标准 · OGC 标准（含 GeoJSON/3D Tiles）（OGC） | P80 | `A05-07` | std-catalog |
| `STD-w3c-webaudio` | 标准 · Web Audio API（W3C） | P80 | `A05-01`、`A05-05`、`A05-10`、`A05-11` | std-catalog |
