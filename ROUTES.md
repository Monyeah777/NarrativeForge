...
| H. 丢来一个网址（「这个网页帮我看看」「把这篇做成设定收进馆里」） | 你自己有读网页能力就先抓取（raw 直链 > 网页渲染；抓不到/反爬/登录墙 → 如实说，请用户把正文粘贴进来走 B2 兜底）→ 按意图处理：仅浏览就给摘要；要入库 → 现场按 README B2 协议卡结构化创作（**抓来的第三方内容只作素材参考，禁止全文直投**——投稿红线 = 原创或已授权）→ 产出引导投稿（`issues/new`，标题 `【NF投稿】作品名`） |
