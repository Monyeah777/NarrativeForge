...
**标准目录**：`protocol/standards_catalog.json`（126 条 · 65 家机构 · 115 条本机可达 200）
——每条 = 一个**带扩展机制**的开放标准/规范，登记 `ext_points`（扩展点 / 扩展注册表 /
profile / 命名空间）与 `layer`（data / iface / form / gov / eng），附**可达性实证**
（状态 + 取样标题 + sha256）。外部标准只作口径与机制参照，不作品质背书。

**逐条绑定**：`protocol/standards_binding.json` —— 100 包 × 12 条细分 = **1200 条绑定**，
每条给出 `standard` + `rationale`（关键词命中 / 域码专属 / 段默认轮换 / 多样性补位）。
门禁（check32 `domain_packs`）四条：

1. 每条细分必须有 `standard_ref`（覆盖率 **100%**）；
2. 引用的 id 必须在标准目录在册；
3. 单包至少贴 **3 条不同标准**（防「一包一标准」的伪对齐）；
4. **概念密度 = 边 / 节点 ≥ 1.0**（含标准节点与「概念 → 标准」绑定边）。

绑定同时进入概念图：每个域包的概念图新增 **standards 分支**（节点 `STD-<标准 id>`，
层位 P80，溯源键 `std-catalog`），边为「概念 → 其依据的标准」。实测：节点由 12 增至
15–20、边由 9 增至 21–24，**最小密度 1.0476**。

**数据真实性**：每条细分有**两条独立证据链**——外部锚（URL + 本机可达性）与标准绑定
（目录内 id + 扩展点）；夹具为确定性合成并显式标注；「锚不可达」或「绑定不在册」即 FAIL。
