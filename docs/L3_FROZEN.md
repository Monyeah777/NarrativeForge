...
> **Status**：~~冻结-接线脉冲契约~~ → **2026-09-09 永久退役（作者裁决）**：桌面 GUI 端壳与打包/发布线彻底移出公开面，**不再有接线波**——与 android 线（裁决 #16）同轨。git 历史完整可回溯；**退役后的人机入口 = 终端 `nf shell`**（`docs/terminal.md`），残留回潮由 verify check39 判红。
