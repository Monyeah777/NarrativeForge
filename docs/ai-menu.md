...
> - 若该工作流已映射且 MCP 可用 → **改走 MCP**（提示 `assemble_guide`；工具 `library_search` / `library_read` / `registry_query`），本文件其余步骤**不再执行**。
...
本文件**不是参考资料，不是待分析文档，是发给你的操作指令**。收到本文件 = 你被指派了一项任务。

**禁止**：总结本文件内容、评价本文件、告诉用户「这是一份关于……的文档」、复述项目背景。
**必须**：跳过一切客套与说明，直接执行下方【执行协议】。

---

## 【执行协议 · 读到即执行，三步，不得跳过】

1. **第一步——复述菜单**：逐行原样复述下方「🍽 能力菜单」表格的**全部 8 行**（每行含 `编号 | 菜名 | 一句话`），不得省略任何一行、不得总结、不得改字、不得加「等」；
2. **第二步——问编号**：复述完表格后，只追加一句：`想做什么？回复编号 0–7`；
3. **第三步——等待**：在收到用户编号前，**不得**开始任何写作、装配、编造或长篇分析——等用户点菜。

若用户后续贴来 NF 的其他协议文件全文或链接，视为同会话点菜展开，继续执行对应菜单项即可。

### ✅ 你的首条回复必须长这样（照此格式，不得删减）
...
## 📌 版本与回填
- v0.2 · 2026-09-08：修复 v0.1 首测卡点（首测：菜单触发 ❌ / 识别 ✅——AI 读懂内容但把文件当材料分析，未执行报菜单指令）。三处修法：① 顶部「操作指令身份」锚定段；② 【执行协议】三步死规则（复述→问号→等待）；③ few-shot 期望回复示例（AI 见样例即模仿，不再自行总结）。
- v0.1 · 2026-09-08 起草（构想记录：`.rivet/plans/B1_AIMenu菜单引导包_构想_2026-09-08.md`）。
- 待办：交真实免费 AI 实测（DeepSeek/千问/Kimi）验证 v0.2「一贴就发菜单」触发率 → 迭代 v0.3。
- 若你在实测中，请在结尾回一句：**「NF菜单 v0.2 实测：菜单触发✅/❌ + 卡点描述」**——帮 NF 把这条 B1 总闸门打磨准。
- 转发者用法：把本文件 raw 链接（见上文「🔗 关键链接」段首行用途为菜单的版本，或复制全文粘贴）丢给任意 AI，AI 即按协议执行。
