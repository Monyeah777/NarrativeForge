...
> **状态**：公开化文档已随基础层 v2.7 落地；**E3 协议级装载实测已于 2026-09-08 通过**（stdio JSON-RPC 帧序列直测五方法全绿，见文末记录）——发现并修复 P06 管线两处实现缺口（缺闭合 ``` / 缺 techdoc 声明）。**宿主类客户端（Claude Desktop 等 GUI 装载）仍建议用户侧补录**，补录前 README 对 MCP 的宣告维持现状（S13 门按 docs_external-validation-v2.7.md 规则开）。
