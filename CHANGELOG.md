...
- **v2.1.0-B4 市场协议 CLI 先行（方案 31，本批）**：check15 ②③ 判据提为可 import 库——新增 `desktop/src/core/market_analyzer.py`（`dependencies` 依赖闭包无环/叶⊆官方13/源包嵌套 references + `conflicts` 挂载层键归一 default 交集）+ nf.py `market <包目录>` 子命令（登记状态 + 依赖闭包 + 冲突预检，CLI 先行门禁前移）。**瑶光发现**：verify.sh check15 ② 源包嵌套检查误取 dependencies.references 恒空（references 在 package 层）——本库按真实层级修正，verify 死检查记 ROADMAP 待修。verify PASS=24 + 单测 184 绿。
### Changed
