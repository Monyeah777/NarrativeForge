...
- **loader 覆盖率达标 + 健壮性修复（W27）**：补 fallback 子集解析/禁用 yaml 回退/discover/边界测试（暴露真 bug：yaml 块为标量时 AttributeError，已修）；逐模块覆盖率 **min 30→50（58 文件 0 低档）**；verify PASS=49 保持。
...
- **CLI 工具链顶尖化（内部工程波次 3 · 机器可读铺满 + 帮助页质量）**：`asset ls` / `asset inventory` / `module ls` 增 `--json`；全部 34 个子命令帮助页补 description 行（`nf help <cmd>` / `--help` 可读性）；verify PASS=49 保持。
...
- **D3 ruff 语法级 lint**：`ruff.toml`（E9/F63/F7/F82）+ `.github/workflows/lint.yml` 独立 job；修复 `market_analyzer.py` 未导入 `Optional`（F821 真问题）。
- **D4 日志级别体系**：`desktop/src/core/plog.py`（logging → stderr，诊断不污染 stdout 协议面）+ `mcp_runtime.main` 启动/退出日志；CLI 用户面 print 保留为输出协议。
...
- W5：ruff 本机冒烟全绿（0.16.6）+ py_compile 通过；单测 406 保持；verify PASS=39 不破（配置/工具层新增，未动协议/门禁）。
